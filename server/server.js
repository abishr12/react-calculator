...
function requestHandler(request, response) {
  ...
  let requestedResource = path.join(__dirname, // current directory where server.js is found
  ...
  fs.exists(requestedResource, function (exists) {
    ...
    // Finally, we read the requested file (asynchronously) and send it's content to
    // the client
    fs
      .readFile(requestedResource, // path to requested resource
          "binary", // read the requested resource as a binary file
          function (err, file) { // call back function to handle end of file reading

        // If an error occured while reading the file, send the error message with a
        ...
        // Helper object to map requested content types (extension) to response mime
        // types
        ...
        // if the requested resource maps to any of our content type extension, then set
        // the Content-Type field for our response headers.
        if (contentType) {
...
// create an instance of our httpServer and passing in our request handler
// callback
